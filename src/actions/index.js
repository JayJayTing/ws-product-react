...
import { GET_HOURLY_EVENTS, GET_DAILY_EVENTS, GET_HOURLY_STATS, GET_DAILY_STATS, GET_POI } from '../actions/types';

export const getHourlyEvents = id => async dispatch => {
	let response = await axios.post('http://localhost:5555/events/hourly', { id });
...
export const getHourlyStats = id => async dispatch => {
	let response = await axios.post('http://localhost:5555/stats/hourly', { id });
